...
# Configuration
IMAGE_NAME="ego_dynhamr:latest"
CONTAINER_NAME="ego-dynhamr"
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DOCKERFILE="${PROJECT_ROOT}/docker/Dockerfile.dynhamr"
...
build_image() {
    ...
    print_info "Building Dyn-HaMR image: ${IMAGE_NAME}"
...
case "$1" in
    ...
    *)
        if [[ -z "$1" ]]; then
            ...
            print_info "  $0 run python demo.py # Run a specific command"
        fi
        ;;
esac
